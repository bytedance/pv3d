...
from lib.utils.distributed import reduce_dict, reduce_tensor
from lib.utils.general import scalarize_dict_values
...
class Meter:
    ...
    def update_from_report(self, report, should_update_loss=True):
        ...
        if hasattr(report, "discriminator_acc"):
            acc_dict = report.discriminator_acc
            reduced_acc_dict = reduce_dict(acc_dict)
        ...
        with torch.no_grad():
            ...
            if hasattr(report, "discriminator_acc"):
                acc_dict = scalarize_dict_values(reduced_acc_dict)
                meter_update_dict.update(**acc_dict)
