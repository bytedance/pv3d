...
# DEBUG_WEIGHT = bool(int(os.environ.get("DEBUG_WEIGHT", False)))
# DEBUG_PERTURB = bool(int(os.environ.get("DEBUG_PERTURB", False)))
# print(f"[Debug] DEBUG_WEIGHT: {DEBUG_WEIGHT} DEBUG_PERTURB: {DEBUG_PERTURB}")
...
def fancy_integration(rgb_sigma, rays_d_cam, z_vals, pts, device, noise_std=0.5, last_back=False, white_back=False, max_depth=None, clamp_mode=None, fill_mode=None, return_xyz=False):
    """Performs NeRF volumetric rendering."""

    rgbs = rgb_sigma[..., :-1] # N x hw x step x C
    sigmas = rgb_sigma[..., -1:] # N x hw x step x 1

    deltas = z_vals[:, :, 1:] - z_vals[:, :, :-1] # N x hw x step x 1
    deltas = deltas * torch.norm(rays_d_cam, p=2, dim=-1, keepdim=True).unsqueeze(2)
    ...
    weights = alphas * torch.cumprod(alphas_shifted, -2)[:, :, :-1]  # N x hw x step x 1
    # if DEBUG_WEIGHT:
    #     weights = 0*weights + 1.0 / z_vals.shape[2] # Equalized weights !!!
    weights_sum = weights.sum(2)  # N x hw x 1

    if last_back:
        weights[:, :, -1] += (1 - weights_sum)

    rgb_final = torch.sum(weights * rgbs, -2) # N x hw x C
    depth_final = torch.sum(weights * z_vals, -2) # N x hw x 1

    if white_back:
        rgb_final = rgb_final + 1-weights_sum
    if max_depth:
        depth_final = depth_final + (1-weights_sum) * max_depth
    ...
    if return_xyz:
        xyz = torch.sum(weights * pts, -2)
        img_size = int(math.sqrt(xyz.size(1)))
        xyz = xyz.reshape(xyz.size(0), img_size, img_size, -1)
        # mask = weights.reshape(weights.size(0), img_size, img_size, -1, 1)[..., -1, :]
        # import cv2
        # cv2.imwrite('test.jpg', mask[0,...,0].cpu().numpy()*255)
    else:
        xyz = None

    return rgb_final, depth_final, weights, xyz
...
def get_initial_rays_trig(n, num_steps, device, fov, resolution, ray_start, ray_end):
    ...
    x, y = torch.meshgrid(torch.linspace(-1, 1, W, device=device),
                          ...
                          indexing="ij")
    x = x.T.flatten() # x horizontal
    y = y.T.flatten() # y vertical
    z = -torch.ones_like(x, device=device) / np.tan((2 * math.pi * fov / 360)/2)

    # rays_d_cam = normalize_vecs(torch.stack([x, y, z], -1)) # hw x 3
    rays_d_cam = torch.stack([x, y, z],-1) / torch.abs(z.unsqueeze(1)) # normalize by z
    ...
    points = torch.stack(n*[points]) # n x hw x step x 3
    z_vals = torch.stack(n*[z_vals]) # n x hw x step x 1
    rays_d_cam = torch.stack(n*[rays_d_cam]).to(device) # n x hw x 3
...
def transform_sampled_points(points, z_vals, ray_directions, device, h_stddev=1, v_stddev=1, h_mean=math.pi * 0.5, v_mean=math.pi * 0.5, camera=None, mode='normal', is_train=False):
    ...
    if camera is not None:
        cam2world_matrix = camera
    else:
        # ignore pitch, yaw
        camera_origin, _, _ = sample_camera_positions(n=points.shape[0], r=1, horizontal_stddev=h_stddev, vertical_stddev=v_stddev, horizontal_mean=h_mean, vertical_mean=v_mean, device=device, mode=mode)
...
def sample_camera_positions(device, n=1, r=1, horizontal_stddev=1, vertical_stddev=1, horizontal_mean=math.pi*0.5, vertical_mean=math.pi*0.5, mode='normal'):
    ...
    if mode == 'uniform':
        ...
        # g.manual_seed(123)
        # theta = torch.tensor([[1.5374]], device=device).repeat(n, 1)
        # phi = torch.tensor([[1.3563]], device=device).repeat(n, 1)
