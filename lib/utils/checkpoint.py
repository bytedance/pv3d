...
from lib.utils.configuration import get_env, load_yaml
from lib.utils.distributed import is_master, synchronize, get_current_device
from lib.utils.general import PathManager, updir
...
def _hack_imports():
    ...
    sys.modules["pythia"] = importlib.import_module("lib")
    sys.modules["pythia.utils.configuration"] = importlib.import_module(
        "lib.utils.configuration"
    )
...
def _load_pretrained_model(model_name_or_path, *args, **kwargs):
    if PathManager.exists(model_name_or_path):
        ...
        raise FileNotFoundError(f"{model_name_or_path} not found.")
    ...
    assert (
        len(ckpts) == 1
    ), "None or multiple checkpoints files. It doesn't know what to do."
...
def _load_pretrained_backbone(checkpoint_path, *args, **kwargs):
    with PathManager.open(checkpoint_path, "rb") as f:
        ckpt = torch.load(f, map_location=lambda storage, loc: storage)
    return ckpt
...
class Checkpoint:
    def __init__(self, trainer):
        ...
        self.save_dir = get_env(key="save_dir")
        ...
        self.repo_path = updir(os.path.abspath(__file__), n=2)
        self.git_repo = None
    ...
    def load_state_dict(self):
        ...
        if (
            ...
            # resume_file doesn't exist, try from zoo now
                ...
                logger.warn(f"{ckpt_config.resume_file} doesn't exist")
    ...
    def _load(self, file, force=False, load_zoo=False, load_pretrained=False):
        ...
        if len(pretrained_state_mapping.items()) == 0:
            state_dict = {k:v for k, v in state_dict.items() if "deform_field" not in k}
        ...
        logger.info(f"Current num samples: {self.trainer.num_samples}")
    ...
    def _load_optimizer(self, ckpt):
        if "optimizer" in ckpt:
            try:
                for group in self.trainer.optimizer:
                    self.trainer.optimizer[group].load_state_dict(ckpt["optimizer"][group])
            except ValueError:
                logger.info(
    ...
    def _load_counts_and_lr_scheduler(self, ckpt):
        ...
        self.trainer.num_samples = ckpt.get(
                "num_samples", self.trainer.num_updates * ckpt["config"]["training"]["batch_size"]
            )
        ...
        registry.register("num_samples", self.trainer.num_samples)
    ...
    def save(self, update, iteration=None, update_best=False):
        ...
        if data_parallel is True:
            model = model.module
            # # NOTE: check ddp consistency
            # for m in ["generator", "generator_ema", "img_discriminator"]:
            #     check_ddp_consistency(eval(f"model.{m}"), ignore_regex=r'.*\.[^.]+_(avg|ema)')
        
        ckpt = {
            "model": model.state_dict(),
            "optimizer": {group: self.trainer.optimizer[group].state_dict() for group in self.trainer.optimizer},
