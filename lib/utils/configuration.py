...
from lib.utils.general import Timer, PathManager
...
def load_yaml(f):
    try:
        ...
        relative = os.path.abspath(os.path.join(get_root(), f))
    ...
    for include in includes:
        ...
        # If path doesn't exist relative to root, try relative to current file
        if not PathManager.exists(include):
            raise FileNotFoundError(f"Include {include} not found")
...
def get_default_config_path():
    configs_dir = os.path.join(get_root(), "configs")
...
def get_cache_dir():
    ...
    # If cache_dir path exists do not join to root
...
def resolve_cache_dir(env_variable="CACHE_DIR", default="."):
    ...
    if not PathManager.exists(cache_path):
        try:
            ...
            cache_path = os.path.join(get_root(), ".cache")
            PathManager.mkdirs(cache_path)
...
def resolve_trial(env_variable="ARNOLD_OUTPUT", default=None):
    arnold_output = os.getenv(env_variable)
    pattern = re.compile(r".*?/trials/(\d{6,9})/output")
    try:
        return pattern.search(arnold_output).groups()[0]
    except:
        return default


def resolve_cuda_env(env_variable="CUDA_VISIBLE_DEVICES", default=None):
    cuda_visible_devices = os.getenv(env_variable)
    if cuda_visible_devices is not None:
        cuda_visible_devices = list(map(int, cuda_visible_devices.split(",")))
    else:
        cuda_visible_devices = list(range(max(1, torch.cuda.device_count())))
...
class Configuration:
    ...
    def _build_dataset_config(self, config):
        ...
        for dataset in datasets:
            builder_cls = registry.get_dataset_builder_class(dataset)
    ...
    def _register_resolvers(self):
        ...
        OmegaConf.register_new_resolver("resolve_trial", resolve_trial)
        OmegaConf.register_new_resolver("resolve_cuda_env", resolve_cuda_env)
