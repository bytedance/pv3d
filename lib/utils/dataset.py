...
def fetch_video_clip(video, num_frames, dataset_type, bi_frame=False):
    sample = dict()
    # try:
    if dataset_type != "train":
        frames = np.zeros(shape=(num_frames, 224, 224, 3))
        sample["frames"] = frames
    else:
        reader = VideoReader(open(video, 'rb'), num_frames, bi_frame=bi_frame)
        sample = reader.sample()
    # except:
    #     raise RuntimeError(f"broken videos {video}")
    return sample


def fetch_aligned_video_clip(video, num_frames, dataset_type):
    if dataset_type != "train":
        frames = np.zeros(shape=(num_frames, 256, 256, 3))
    else:
        videos = sorted(glob.glob("{}/*.jpg".format(video[:-4].replace("dev/mp4", "aligned_faces_70000"))))
        offset = torch.randint(max(1, len(videos)-num_frames-1), [1]).item()
        frames = np.array([cv2.imread(img)[:,:,::-1] for img in videos[offset:offset+num_frames]])
    sample = dict()
    sample["frames"] = frames
    return sample
...
class VideoReader():
    ...
    """
    def __init__(self, video, num_frames, decode_lossy=False, audio_resample_rate=None, bi_frame=False):
        """
        Arguments:
