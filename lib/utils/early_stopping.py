...
from lib.utils.configuration import get_global_config, get_env
...
class EarlyStopping:
    ...
    def __init__(
        ...
        datasets = list(get_global_config("datasets"))
        for dataset in datasets:
            if not dataset in self.early_stop_criteria:
                raise RuntimeError(
                "Dataset {} not used in early stop criteria {}".
                format(dataset, self.early_stop_criteria)
                )
        ...
        self.trial = get_global_config("env.trial")
        self._arnold_init()
    ...
    def _arnold_init(self):
        if self.trial is not None:
            cmd = f"hdfs dfs -mkdir -p ${{HDFS}}/output/{self.trial}"
            exec_process(cmd)
    
    def _arnold_sync(self):
        save_items = ["best.ckpt", "videos/fake", "images/fake", "config.yaml"]
        if self.trial is not None:
            save_dir = get_env(key="save_dir")
            for item in save_items:
                local_file = f"{save_dir}/{item}"
                if not os.path.isfile(local_file):
                    cmd = f"hdfs dfs -rm -r ${{HDFS}}/output/{self.trial}/{item}"
                    exec_process(cmd)
                cmd = f"hdfs dfs -put -f {local_file} ${{HDFS}}/output/{self.trial}/"
                exec_process(cmd)