...
from lib.utils.distributed import get_rank, is_master, synchronize
from lib.utils.general import PathManager, Timer, exec_process
...
def setup_output_folder(folder_only: bool = False):
    ...
    env_log_dir = get_env(key="log_dir")
    if env_log_dir:
...
def setup_logger(
    ...
    **kwargs,
    ...
    Args:
        ...
        name (str): the root module name of this logger. Defaults to __name__.
    ...
    # Remove existing handlers to add specific handlers
    if clear_handlers:
...
def setup_very_basic_config(color=True):
    ...
    # log a message even before logging is setup.
...
def log_class_usage(component_type, klass):
    """This function is used to log the usage of different components."""
    identifier = "vgan3d"
...
# ColorfulFormatter is adopted from Detectron2 and adapted
...
class TensorboardLogger:
    ...
    def add_images(self, images, iteration, tag="images"):
        if not self._should_log_tensorboard():
            return

        self.summary_writer.add_images(tag=tag, img_tensor=images, global_step=iteration)

    def add_video(self, video, iteration, tag="video"):
        if not self._should_log_tensorboard():
            return
            
        self.summary_writer.add_video(tag=tag, vid_tensor=video, fps=25, global_step=iteration)
    ...
    def _sync_arnold(self):
        if os.getenv("ARNOLD_OUTPUT"):
            cmd = "hdfs dfs -mkdir {} ${{ARNOLD_OUTPUT}}/".format(self.summary_writer.log_dir)

    def _flush_arnold(self):
        if os.getenv("ARNOLD_OUTPUT"):
            cmd = "hdfs dfs -put -f {}/* ${{ARNOLD_OUTPUT}}/".format(self.summary_writer.log_dir)
            exec_process(cmd)


def log_class_usage(component_type, klass):
    """This function is used to log the usage of different vgan3d components."""
    identifier = "vgan3d"
    if klass and hasattr(klass, "__name__"):
        identifier += f".{component_type}.{klass.__name__}"
    torch._C._log_api_usage_once(identifier)